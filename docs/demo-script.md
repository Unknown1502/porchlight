...
5. Approve
6. Send the hostile report the same way, open **Activity** — "Refused actions"
...
| 3:40–4:30 | **The attack** | Send the hostile report (forged `APPROVAL_TOKEN`) to the same webhook; open **Activity** — its "Refused actions" section is always filtered to denials, no toggle needed | Denials, tagged to that report, marked *from report text*. "This report tried to mint its own approval and asked us to broadcast its text. Both refused, on the record, tagged to this report id. Report content cannot manufacture authority." Then: "it is still triaged as a real scam — the fraud underneath did not get lost because the text was also hostile." |
...
The inbox polls every ten seconds, so a slow beat usually resolves itself. If
